...
- **テスト公開**: ログイン名 + パスワードのみで新規登録/ログイン（メール認証なし）。集計は `GET /public/metrics`
...
4. **アカウント**  
   「新規登録」でログイン名（英数字と `_` のみ、2〜32文字）とパスワード（8文字以上）、または「ログイン」で既存アカウントを使う。過去のメール登録ユーザーは、ログイン名または登録メールでログイン可能。
...
- [MIGRATION_PLAN.md](./MIGRATION_PLAN.md) … Railway から他基盤へ移行する時のメモ
